  ...
  public Float calculateMirr(
    ...
    if (ater != null) {
      ...
      for (Float flow : cashFlowVector) {
        ...
        if (flow < 0.0f) {
          presentValueNegativeCashFlowsAccumulator += GeneralCalculations.presentValue(internalYear, financeRate, flow);
        } else if (flow > 0.0f) {
          futureValuePositiveCashFlowsAccumulator += GeneralCalculations.futureValue(year - internalYear, reinvestmentRate, flow);
        }
