...
public class ExcelAttachment {
  ...
  public static final String OUTPUT_WORKBOOK = "REAP_workbook.xls";
  private FileOutputStream output_workbook_fos;
  private WritableWorkbook workbook;
  private Context context;
  private File outputFile;

  public ExcelAttachment( Context context) {
...
//      workbook.close();
  ...
  private void openFileOutputStream(Context context) {
    try {
      ...
      output_workbook_fos =  context.openFileOutput(OUTPUT_WORKBOOK, Context.MODE_WORLD_READABLE);
    } catch (IOException e) {
      e.printStackTrace();
    }  
  }
  ...
  private void createWorkbook() {
    ...
    try {
      workbook = Workbook.createWorkbook(output_workbook_fos);
  ...
  private void addDataToWorkbook() {
    ...
    for (int i = 2; i < 20; i++) {
      Number nCell = new Number(1, i, dataController.getCalcValue(
          ValueEnum.MODIFIED_INTERNAL_RATE_OF_RETURN, i*12));
      try {
        sheet.addCell(nCell);
