...
class ATCFGraph extends View {
  ...
  private HashMap<Float, Float> atcfGraphMap;
  ...
  private void initView() {
    ...
    atcfGraphMap = new HashMap<Float, Float>();
    gdo = calculationObject.getGdo();
  }

  private void buildNPVGraph() {
    double functionMinY = gdo.getMinAtcf();
    double functionMaxY = gdo.getMaxAtcf();
    double functionMinX = gdo.getMinAtcfYearNumber();
    double functionMaxX = gdo.getMaxAtcfYearNumber();
    ...
    HashMap<Float, Float> atcfDataPoints = gdo.getYearlyAtcf();

    for (HashMap.Entry<Float, Float> entry : atcfDataPoints.entrySet()) {
      ...
      atcfGraphMap.put(xGraphValue, yGraphValue);
    }
  }
...
//    //if we are not looking at the view in Eclipse...
    if (! isInEditMode()) {
      ...
      for (HashMap.Entry<Float, Float> entry : atcfGraphMap.entrySet()) {
        Float xValue = entry.getKey();
        Float yValue = entry.getValue();
